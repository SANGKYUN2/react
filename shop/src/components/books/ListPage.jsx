...
const ListPage = () => {
    ...
    return (
        <div className='my-5'>
            <h1 className='text-center'>도서목록</h1>
            <Row className='mb-2'>
                <Col xs={6} md={5} lg={4}>
                    <form onSubmit={onSubmit}>
                        <InputGroup>
                            ...
                            <Button variant="warning" type='submit'>검색</Button>
                        </InputGroup>
                    </form>
                </Col>
                ...
                {count > 0 &&
                    <Col className='text-end'>
                        <Button onClick={onDeleteChecked} variant='danger'>선택 도서 삭제</Button>
                    </Col>
                }
            </Row>
            {count > 0 ?
                <Table variant='success' striped bordered hover className='align-middle text-center'>
                    ...
                    <tbody>
                        {books.map(book =>
                            <tr key={book.bid}>
                                <td className='text-center'><input onChange={(e) => onChangeSingle(e, book.bid)} checked={book.checked} type="checkbox" /></td>
                                <td>{book.bid}</td>
                                <td><img src={book.image || 'http://via.placeholder.com/120x170'} width="40px" /></td>
                                <td><a href={`/books/update/${book.bid}`}>{book.title}</a></td>
                                <td>{book.fmtprice}원</td>
                                <td>{book.author}</td>
                                <td>{book.fmtdate}</td>
                                <td className='text-center'><Button onClick={() => onDelete(book)}
