...
const ListPage = ({ id }) => {
    ...
    const callAPI = () => {
        ...
        onSnapshot(q, res => {
            ...
            res.forEach(row => {
                count++;
                rows.push({no:count, cid: row.id, ...row.data(), isEllip : true, isEdit:false, text:row.data().content })
            });
            //console.log(rows);
            setTotal(count);
                            ...
                            <Row className='mb-2'>
                                ...
                                {(c.email === sessionStorage.getItem('email')) && !c.isEdit && 
                                    <Col className='text-end'>
                                        ...
                                        variant='outline-danger' size='sm' className='ms-2'>삭제</Button>
                                    </Col>
                                }
                                {(c.email === sessionStorage.getItem('email')) && c.isEdit &&
                                    <Col className='text-end'>
                                        ...
                                        variant='outline-danger' size='sm' className='ms-2'>취소</Button>
                                    </Col>
                                }
                            </Row>
                            {c.isEdit ?
                                <div>
                                    ...
                                    value={c.content} as="textarea" rows={10}/>
                                </div>
                                :
                                <div onClick={()=>onClickContent(c.cid)}
                                className={c.isEllip ? 'ellipsis2':''} style={{ whiteSpace: "pre-wrap", cursor : 'pointer' }}>{c.content}</div>
                            }
                            <hr />
                        </div>
                    )}
